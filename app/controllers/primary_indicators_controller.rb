class PrimaryIndicatorsController < ApplicationController
	...
	def fetch_primary_indicator_data_fast_sql
		...
		if column_name == "ethnic_composition"
			...
			results = {}
			keys.each do |key|
				sql = Arel.sql("unique_georreferenciacions.#{group_field}, 
												AVG((primary_indicators.ethnic_composition->>'#{key}')::numeric)")
				avg_for_key = query.group("unique_georreferenciacions.#{group_field}")
													.pluck(sql)

				avg_for_key.each do |g, avg|
					results[g] ||= {}
					results[g][key] = avg.nil? ? nil : avg.to_f.round(2)
				end
			end

			formatted = results.map do |g, values|
				{ group_field => g }.merge(values)
			end
		else
			results = query.group("unique_georreferenciacions.#{group_field}")
										.average("primary_indicators.#{column_name}")
