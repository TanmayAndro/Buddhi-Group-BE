class CalculateAllPrimaryJob < ApplicationJob
  ...
  #   dane_codes = PrimaryIndicator.distinct.pluck(:dane_code)

  #   dane_codes.each do |dane_code|
  #     indicators = update_primary_indicator(dane_code)
  #     total_processed += 1
  #   end
  ...
  # end
	...
	# 			Arel.sql("SUM(CASE WHEN age_group IN (2,3) AND highest_education = #{NewPersona.defined_enums['highest_education']['Primary basic']} THEN 1 ELSE 0 END)"), # enrolled in primary, # enrolled in primary
  ...
  #   end
	...
	def bulk_update_urbanization_rate
		...
		# Precompute dane_code + total population + urban population
		persona_stats = NewPersona.group(:dane_code_anm).pluck(
			:dane_code_anm,
			Arel.sql("COUNT(*)"), # total persons
			Arel.sql("SUM(CASE WHEN unit_info IN (1,2) THEN 1 ELSE 0 END)") # urban pop
		...
		persona_stats.each do |dane_code, (total_person, urban_pop)|
			# Safeguard: avoid division by zero / NaN
			urbanization_rate =
				if total_person > 0
